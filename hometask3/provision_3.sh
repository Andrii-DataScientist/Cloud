...
&& aws ec2 create-security-group --group-name SecurityGroup --description "My security group" --vpc-id $VPC_ID \
...
&& aws ec2 run-instances --image-id ami-0f91cedb707b09db0 --count 1 --instance-type t2.micro --key-name MyKeyPair --security-group-ids $SG --subnet-id $NET_ID --user-data file://user_script.sh \
