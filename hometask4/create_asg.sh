...
echo $TG_ARN
INST1_ID=$(aws ec2 describe-instances --query 'Reservations[-1].Instances[0].[InstanceId]' --output text)
INST2_ID=$(aws ec2 describe-instances --query 'Reservations[-2].Instances[0].[InstanceId]' --output text)
echo $INST1_ID
echo $INST2_ID
